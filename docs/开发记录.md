...
- **需求**：当前对话没有停止按钮，用户希望能在模型生成过程中取消当前回复，并且取消信号能通过 ACP 发送到 `pi`。
- **改动**：
  - `src/renderer/src/stores/session.ts`：
    - 新增 `cancelRun()` action，调用 `api.nativeCancelRun(currentSession.value.sessionId)`，并立即将 `isGenerating` 置为 `false` 反馈 UI。
  - `src/renderer/src/components/Composer.vue`：
    - 新增 `stop()` 函数，调用 `store.cancelRun()`。
    - `send()` 中增加 `isGenerating` 检查，防止生成中重复发送。
    - 工具栏发送按钮在 `store.isGenerating` 时切换为红色「停止」按钮，图标为实心方块，点击执行 `stop()`。
  - `src/renderer/src/styles/design.css`：
    - 新增 `.send-btn.stop-btn` 样式，背景色使用 `var(--error)`，hover 加深。
  - `src/main/pi-sdk-driver/pi-sdk-driver.ts` / `native-ipc.ts`：
    - `NATIVE_CANCEL_RUN` IPC 已在；`PiSdkDriver.cancelRun` 会按当前驱动模式分发：ACP 模式调用 `AcpDriverBridge.cancel(sessionId)`，SDK 模式调用 `SessionSupervisor.cancelRun(sessionId)`。
    - `AcpDriverBridge.cancel` 调用 `this.piAgent.cancel({ sessionId })`，经 ACP 协议转发到 `pi-acp` 的 `PiAcpAgent.cancel`，最终调用 `PiAcpSession.cancel()` 设置 `cancelRequested` 并 `proc.abort()`。
- **构建验证**：`npx tsc --noEmit` 与 `npm run build` 均通过。

### 2026-08-05：优化 skill 发送后在聊天记录中的展示

- **需求**：用户发送 skill 后，聊天记录中仍然把 `/skill:<name> <args>` 当作纯文本显示，希望能够像「调用 XXX 智能体」一样做视觉优化。
- **改动**：
  - `src/renderer/src/components/ChatView.vue`：
    - `ChatItem` 新增 `skillName?: string`。
    - 新增 `parseSkillPrefix()`，支持解析 `/skill:` 与全角 `/skill：` 前缀，提取技能名与参数。
    - `parseUserContent()` 在剥离 `<<<SYSTEM>>>` 智能体提示块后，继续解析 skill 前缀：如果命中，则返回 `{ realInput: 参数, skillName: 技能名, agentName }`。
    - 用户消息气泡中新增 `<div v-if="m.skillName" class="msg-skill-note">调用 {{ m.skillName }} 技能</div>`，与智能体调用提示并列。
  - `src/renderer/src/styles/design.css`：
    - 将 `.msg-agent-note` 与 `.msg-skill-note` 共用基础样式。
    - `.msg-skill-note` 使用 `var(--warning)` 颜色，与技能芯片主题一致。
- **构建验证**：`npx tsc --noEmit` 与 `npm run build` 均通过。

### 2026-08-05：修复 Windows baseline Bun 构建失败

- **问题**：Windows 构建在 `bun build --compile --target=bun-windows-x64-baseline` 时偶发 `Failed to extract executable for 'bun-windows-x64-baseline-v1.3.14'. The download may be incomplete.`，而 Linux/macOS 构建正常。
- **排查**：
  - 本地通过代理 `127.0.0.1:7788` 拉取 `bun-v1.3.14` 与 `bun-v1.3.13` 的 `bun-windows-x64-baseline.zip` 均可正常解压，包本身没有损坏。
  - 用普通 `bun-windows-x64` 做 cross-target compile 到 baseline 时，1.3.14 成功但耗时 189s（说明是在下载 baseline 包）；直接预先把 baseline `bun.exe` 替换为 host 后编译，耗时约 1s 且不依赖 cross-target 下载。
  - 判断为 Bun 在 Windows 上 cross-target compile 下载/解压 baseline runtime 的稳定性问题，而非某个版本包损坏。
- **改动**（仅 `build-win`）：
  - `.github/workflows/build.yml`：
    - Windows 的 `Setup Bun` 从 `latest` 固定为 `1.3.13`。
    - 在 `Recompile Pi with baseline Bun target` 前新增 `Pre-fetch baseline Bun runtime (Windows)` 步骤：
      - 用 `curl --retry` 下载与当前 `bun --revision` 一致的 `bun-windows-x64-baseline.zip`。
      - 解压后用 baseline `bun.exe` 替换 `~/.bun/bin/bun.exe`。
      - 使 host 与 target 同为 baseline，后续 `bun build --compile` 不再触发 cross-target 下载，避免 extract 失败。
- **验证**：
  - 本地 1.3.13 baseline 包可正常解压；替换后 `bun build --compile --target=bun-windows-x64-baseline` 成功。
  - `node -e "require('js-yaml').load(...)"` 验证 `.github/workflows/build.yml` 语法解析正常。
